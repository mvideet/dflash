"""Smoke test spine-first vs full-tree verify.

If spine_first is implemented correctly, tau should match the full-tree baseline
(spine-first selects the same accept path; only saves time on full-accept rounds).
"""
...
def main():
    ...
    parser.add_argument("--output-json", type=str, default="logs/spine_first.json")
    ...
    for label, B, mode_kwargs in CONFIGS:
        ...
        try:
            ...
            row = {"label": label, "B": B, "M": M, "v7_tps": round(v7_tps, 1),
                   "vanilla_tps": round(van_by_B[B], 1),
                   "speedup": round(speedup, 3), "tau": round(tau, 3)}
            rows.append(row)
            print(f"  {label:>22}: {v7_tps:>7.1f} tok/s, tau={tau:.2f}, "
                  f"speedup={speedup:.3f}×", flush=True)
        except Exception as e:
            rows.append({"label": label, "B": B, "error": str(e)[:300]})
            print(f"  {label:>22}: CRASHED — {e}", flush=True)
            import traceback; traceback.print_exc()

    print("\n=== summary ===", flush=True)
    for r in rows:
        if "error" in r:
            print(f"  {r['label']:>22}: CRASHED")
            continue
