#!/bin/bash
# Iter 19: CDS — corrective bonus log(P_emp/P_marg) on top of draft q.
...
run_cfg() {
    local TAG=$1; local DS=$2; local N=$3; local MTS=$4; local LAM=$5
    local LOG="${LOGDIR}/${TAG}.log"
    if [ -f "${LOG}" ] && grep -q "Decoding speedup:" "${LOG}"; then
        echo "[skip] ${TAG}"
        return 0
    fi
    echo "[run ] ${TAG} B=${MTS} λ=${LAM}"
    timeout 1800 \
...
# 4 datasets × B=128 × 3 λ values (0.5, 1.0, 2.0).
for DS in math500 mt-bench gsm8k humaneval; do
    N=${NSAMPLES[${DS}]}
    echo "=== ${DS} (N=${N}) ==="
    for LAM in 0.5 1.0 2.0; do
        run_cfg "iter19_${DS}_b128_lam${LAM}" ${DS} ${N} 128 ${LAM}
    done
done

echo ""
echo "=== ITER 19 CDS Summary vs CGDB baseline ==="
for DS in math500 mt-bench gsm8k humaneval; do
