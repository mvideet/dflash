...
run_cfg() {
    local TAG=$1; local DS=$2; local N=$3; local MTS=$4
    local LOG="${LOGDIR}/${TAG}.log"
    if [ -f "${LOG}" ] && grep -q "Decoding speedup:" "${LOG}"; then
        echo "[skip] ${TAG}"
        return 0
    fi
    echo "[run ] ${TAG} B=${MTS} adapt_sd"
    timeout 1800 \
...
for DS in math500 mt-bench gsm8k humaneval; do
    ...
    echo "=== ${DS} (N=${N}) ==="
    for B in 128 192; do
        run_cfg "iter17_${DS}_b${B}_adaptsd" ${DS} ${N} ${B}
    done
done

echo ""
echo "=== ITER 17 Adaptive SD Summary vs CGDB baseline ==="
for DS in math500 mt-bench gsm8k humaneval; do
    for B in 128 192; do
