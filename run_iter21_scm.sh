#!/bin/bash
# Iter 21: SCM — sibling-coverage maximization. Bonus α for novel (d, j) buckets.
...
run_cfg() {
    local TAG=$1; local DS=$2; local N=$3; local MTS=$4; local A=$5
    local LOG="${LOGDIR}/${TAG}.log"
    if [ -f "${LOG}" ] && grep -q "Decoding speedup:" "${LOG}"; then
        echo "[skip] ${TAG}"
        return 0
    fi
    echo "[run ] ${TAG} B=${MTS} α=${A}"
    timeout 1800 \
...
for DS in math500 mt-bench gsm8k humaneval; do
    ...
    for A in 0.5 1.0 2.0; do
        run_cfg "iter21_${DS}_b128_a${A}" ${DS} ${N} 128 ${A}
    done
done

echo ""
echo "=== ITER 21 SCM Summary vs CGDB baseline ==="
for DS in math500 mt-bench gsm8k humaneval; do
