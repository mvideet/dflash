"""Multi-seed confidence intervals on the headline CDDT claims.

Runs CDDT γ=0.85 vs baseline on math500/aime24/gsm8k at 3 different shuffles
(seed=0, 1, 2) × N=20 prompts. Reports mean ± std of speedup gain.

Goal: distinguish robust signal from sample-specific noise.
"""
...
def main():
    ...
    parser.add_argument("--output-json", type=str, default="logs/multiseed.json")
    ...
    for ds_name in DATASETS:
        ...
        for seed in SEEDS:
            ...
            for variant, var_kw in [("v7", dict()), ("cddt_g85", dict(score_decay=0.85))]:
                torch.manual_seed(seed)
                ...
                print(f"  seed={seed} {variant:>10}: tau={tau:>5.2f} speedup={speedup:>5.3f}×",
                      flush=True)

    print("\n=== Multi-seed gain summary (γ=0.85 vs v7) ===", flush=True)
    ...
    for ds_name in DATASETS:
        ...
        for seed in SEEDS:
            ...
            cddt = next((r for r in rows if r['dataset']==ds_name and r['seed']==seed and r['variant']=='cddt_g85'), None)
            if base and cddt:
                g = 100 * (cddt['speedup'] - base['speedup']) / base['speedup']
                gains.append(g)
                line += f"{g:>+5.1f}%  "
            else:
                line += f"{'--':>5}  "
