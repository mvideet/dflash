#!/bin/bash
# Iter 13: PLDG. Smooth power-law deep gating.
...
# PLDG configs: 3 power-law exponents.
declare -a CONFIGS=(
    "p0p5 --v8-pldg-p 0.5 --v8-pldg-shallow-depth 4"
    "p1   --v8-pldg-p 1.0 --v8-pldg-shallow-depth 4"
    "p1p5 --v8-pldg-p 1.5 --v8-pldg-shallow-depth 4"
)
...
for DS in math500 mt-bench gsm8k humaneval; do
    ...
    for B in 128 192; do
        for CFG in "${CONFIGS[@]}"; do
            ...
            run_cfg "iter13_${DS}_b${B}_${CFG_NAME}" ${DS} ${N} ${B} "${CFG_FLAGS}"
        done
    done
done

echo ""
echo "=== ITER 13 PLDG Summary vs CGDB baseline ==="
for DS in math500 mt-bench gsm8k humaneval; do
    for B in 128 192; do
        ...
        for CFG_NAME in p0p5 p1 p1p5; do
            LOG="${LOGDIR}/iter13_${DS}_b${B}_${CFG_NAME}.log"
            [ -f "${LOG}" ] || continue
