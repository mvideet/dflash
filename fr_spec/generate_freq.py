...
def main(args):
    if args.dataset_name:
        # Load from HuggingFace (e.g. cerebras/SlimPajama-627B with train[:1%])
        if args.sample_pct is not None and args.shuffle_seed is not None:
            # Random sample: load full split, shuffle, take sample_pct
            base_split = args.dataset_split.split("[")[0]  # "train" from "train[:1%]"
            logger.info(f"Loading {args.dataset_name} {base_split}, then random {args.sample_pct*100:.1f}% (seed={args.shuffle_seed})")
            ds = load_dataset(args.dataset_name, split=base_split, trust_remote_code=True)
            ds = ds.shuffle(seed=args.shuffle_seed)
            n = int(len(ds) * args.sample_pct)
            ds = ds.select(range(n))
            logger.info(f"Selected {n} samples")
        else:
            # Deterministic slice (e.g. train[:1%] = first 1%)
            logger.info(f"Loading HuggingFace dataset: {args.dataset_name}, split={args.dataset_split}")
            ds = load_dataset(args.dataset_name, split=args.dataset_split, trust_remote_code=True)
    else:
    ...
    num_processes = min(args.num_proc, max(1, len(dataset_list)))  # avoid more workers than samples
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", type=str, default=None, help="Output name for saved files (default: from model_path).")
    parser.add_argument(
        "--model_path", type=str, default="Qwen/Qwen3-4B", help="Model path for tokenizer (e.g. Qwen/Qwen3-4B)."
    )
    parser.add_argument(
        "--dataset_name", type=str, default=None,
        help="HuggingFace dataset name (e.g. cerebras/SlimPajama-627B). If set, uses this instead of dataset_path.",
    )
    parser.add_argument(
        "--dataset_split", type=str, default="train[:1%]",
        help="Dataset split. 'train[:1%%]' = first 1%% (deterministic). For random 1%%, use 'train' + --sample_pct 0.01 --shuffle_seed 42.",
    )
    parser.add_argument(
        "--sample_pct", type=float, default=None,
        help="Random sample this fraction of data (e.g. 0.01 for 1%%). Requires --shuffle_seed. Ignores slice in dataset_split.",
    )
    parser.add_argument(
        "--shuffle_seed", type=int, default=None,
        help="Shuffle dataset with this seed before sampling. Use with --sample_pct for random subset.",
    )
    parser.add_argument(
        "--dataset_path", type=str, default=None,
        help="Local path for load_from_disk (used when dataset_name is not set).",
    )
    parser.add_argument("--vocab_size", nargs="+", type=int, default=[65536], help="The vocab sizes to process.")
    ...
    if not args.dataset_name and not args.dataset_path:
        raise ValueError("Must provide either --dataset_name (HuggingFace) or --dataset_path (local).")
