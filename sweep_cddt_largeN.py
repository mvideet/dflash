"""Final cross-dataset confidence run: CDDT static at N=24, B=4.

Resolves the variance ambiguity from earlier runs (CDDT showed -0.5% to +1.4%
on gsm8k across N=12 runs). This larger-N run gives a more reliable estimate.
"""
...
def main():
    ...
    parser.add_argument("--output-json", type=str, default="logs/cddt_largeN.json")
    ...
    for ds_name in DATASETS:
        print(f"\n========== Dataset: {ds_name} (N={args.max_samples}) ==========", flush=True)
        try:
        ...
        for variant, var_kw in [("v7_baseline", dict()), ("cddt_static", dict(score_decay=0.85))]:
            torch.manual_seed(0)
            ...
            print(f"  {variant:>13}: tau={tau:>5.2f} speedup={speedup:>5.3f}× tps={v7_tps:.0f} "
                  f"(n_rounds={len(accs)})", flush=True)

    print("\n=== Final cross-dataset confidence (N=24, B=4) ===", flush=True)
    print(f"  {'dataset':>12}  {'baseline':>10}  {'cddt':>10}  {'gain%':>8}  {'rounds':>8}", flush=True)
    for ds_name in DATASETS:
        b = next((r for r in rows if r['dataset'] == ds_name and r['variant'] == 'v7_baseline'), None)
        c = next((r for r in rows if r['dataset'] == ds_name and r['variant'] == 'cddt_static'), None)
        if b and c:
            gain = 100 * (c['speedup'] - b['speedup']) / b['speedup']
            print(f"  {ds_name:>12}  {b['speedup']:>9.3f}× {c['speedup']:>9.3f}× {gain:>+7.2f}%  "
                  f"{c['n_rounds']:>8}", flush=True)
