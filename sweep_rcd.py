"""Rank-Conditional Decay (RCD) — decay applied ONLY to post-deviation depths.

Hypothesis: rank-0 chain should have uniform weights (heap correctly prefers
it). The "shallow-focus" effect of geometric decay comes from down-weighting
DEEP DEVIATED extensions, not deep contributions everywhere. RCD isolates
this: weight(d, on_rank0) = 1.0; weight(d, deviated) = decay^(d - dev_depth).

If RCD gains match or exceed decay_085, it confirms that the mechanism is
"don't trust deep extensions of deviated paths" — and the rank-0 chain
...
VARIANTS = [
    ...
    ("rcd_0.9",      dict(rank_conditional_decay=0.9)),
    ("rcd_0.8",      dict(rank_conditional_decay=0.8)),
    ("rcd_0.7",      dict(rank_conditional_decay=0.7)),
    ("rcd_0.6",      dict(rank_conditional_decay=0.6)),
]
...
def main():
    ...
    parser.add_argument("--output-json", type=str, default="logs/rcd_sweep.json")
    ...
    for label, B, mode_kwargs in CONFIGS:
        ...
        for variant, var_kwargs in VARIANTS:
            ...
            try:
                ...
                print(f"  {variant:>13}: CRASHED — {e}", flush=True)

    print("\n=== speedup matrix ===", flush=True)
    print(f"  {'variant':>13}  " + "  ".join(f"{l:>6}" for l, _, _ in CONFIGS), flush=True)
    by_v = {(r['variant'], r['B']): r['speedup'] for r in rows if 'speedup' in r}
    for v_name, _ in VARIANTS:
        line = f"  {v_name:>13}  " + "  ".join(f"{by_v.get((v_name, B), float('nan')):>6.3f}" for _, B, _ in CONFIGS)
    ...
    print(f"  {'variant':>13}  " + "  ".join(f"{l:>6}" for l, _, _ in CONFIGS), flush=True)
    for v_name, _ in VARIANTS:
        line = f"  {v_name:>13}  " + "  ".join(f"{by_v_tau.get((v_name, B), float('nan')):>6.2f}" for _, B, _ in CONFIGS)
        print(line, flush=True)
